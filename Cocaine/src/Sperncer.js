var sperncer= new Person("Sperncer Jernsern",res.sperncerPNG);
//posts
sperncer.posts.push(["butts butts butts"]);
sperncer.posts.push(["Guys you should all come watch me play jazz tonight while unicycling!"]);
sperncer.posts.push(["Okay, Cocos is actually terrible, how do you even make a clipping node??"]);
sperncer.posts.push(["This is social satire"]);
sperncer.posts.push(["dehindabuh"]);
sperncer.posts.push(["ヽ༼ຈل͜ຈ༽ﾉ"]);
sperncer.posts.push(["I've actually been awake for 28 hours now. shit dude"]);
sperncer.posts.push(["satire"]);
sperncer.posts.push(["Jazz Butts: The Musical, featuring 12 different actors portraying alternate Spencer egos, all of whom are a part of a whistling band. Together they must undermine the work of Spanker, a nefarious Spencer doppelganger trying the rid the world of all butts"]);

//create a new conversation to append to the conversation list
//                               |
// this conversations difficulty v

var convo_1_1 = new Conversation(1);
convo_1_1.modules.push(["butts?",
						"ummm what",
						"butts butts butts",
						"no spencer",
						"You're no fun"]);
convo_1_1.modules.push(["I like those",
						"i know",
						"They're great",
						"totally!",
						"You're no fun"]);

sperncer.conversations.push(convo_1_1);

var convo_1_2 = new Conversation(1);
convo_1_2.modules.push(["so what's new?",
						"ugh homework",
						"how are ya?",
						"homework. super busy",
						"I guess you're busy"]);
convo_1_2.modules.push(["don't do it!",
						"why not?",
						"seriously!",
						"why not?",
						"you should listen to me next time..."]);
convo_1_2.modules.push(["Because we're going to Brown Bag!",
						"awesome man!",
						"you coming?",
						"absolutely!",
						"alright well I can't wait"]);

sperncer.conversations.push(convo_1_2);

var convo_1_3 = new Conversation(1);
convo_1_3.modules.push(["hey guess what!",
						"chicken butt",
						"guesssssss",
						"chicken butt?",
						"I hate you"]);
convo_1_3.modules.push(["you know me too well",
						"well duh",
						"I like butts",
						"I know spencer",
						"like wayyy more than I like you. Bye"]);
						
sperncer.conversations.push(convo_1_3);

var convo_1_4 = new Conversation(1);
convo_1_4.modules.push(["I'm bored",
						"you're boring",
						"asdjflkja;lkjfal;kfd",
						"so go do something",
						"You're a butt"]);
convo_1_4.modules.push(["ouch. shots fired.",
						"bang bang",
						"bang bang",
						"pow pow",
						"boooooom *death in a firey explosion*"]);
						
sperncer.conversations.push(convo_1_4);

var convo_2_1 = new Conversation(2);

convo_2_1.modules.push(["Hey did I tell you about the thing?",
						"What thing?",
						"The thing that everyone is talking about?",
						"What is it?",
						"Everyone except you, ya butthole"]);
convo_2_1.modules.push(["the RGS thing!",
						"oh yeah that's gonna be cool!",
						"The Rensselaer Game Showcase",
						"on november 21st?",
						"you better be there! >_<"]);
convo_2_1.modules.push(["yeah it's on november 21st!",
						"I can't wait!",
						"You're gonna go right?",
						"No I'll be playing smash all day",
						"you better be there! >_<"]);
						
sperncer.conversations.push(convo_2_1);
						
var convo_2_2 = new Conversation(2);

convo_2_2.modules.push(["so how's the game dev project?",
						"some stupid facebook game",
						"I bet it's terrible",
						"yeah we're making facebook",
						"I'm better than you"]);

convo_2_2.modules.push(["you're making facebook? wow that's dumb",
						"It wasn't my idea",
						"totally not really cool or anything",
						"jealous? :P",
						"okay I'm totally jealous, I hate my team"]);

sperncer.conversations.push(convo_2_2);						
						
var convo_2_3 = new Conversation(2);

convo_2_3.modules.push(["Dude I just thought.",
						"really? new feeling?",
						"Dude just think about it...",
						"Think about what?",
						"You're not even thinking about it"]);

convo_2_3.modules.push(["...Jazz...",
						"yes...jazz??",
						"*starts whistling jazz music*",
						"Stop that sounds terrible",
						"*walks away from uninterested friend*"]);
convo_2_3.modules.push(["shhh it's just fantastic",
						"ehh...I can't admit that",
						"You're just a hater",
						"yep, that's me.",
						"Haters gonna hate"]);
						
sperncer.conversations.push(convo_2_3);
	
var convo_3_1 = new Conversation(3);	

convo_3_1.modules.push(["DUDE. What time is it?",
						"Like quarter past twelve I think ",
						"WHAT TIME IS IT????",
						"Adventure Time? Come on grab your friends? ",
						"Time to find friends who answer me"]);
						
convo_3_1.modules.push(["NO. IT'S TIME TO TAKE TO THE NEXT LEVEL",
						"What? What does that even mean??",
						"O - POWWERRRR",
						"I think you need to stop playing Pokemon",
						"POWEORWOEROWREOWEROWEROW"]);
						
sperncer.conversations.push(convo_3_1);
					
var convo_4_1 = new Conversation(4);	

convo_4_1.modules.push(["Wait do you know what I just Realized?",
						"No. What did you realize on this fine evening?",
						"I just had a BRILLIANT idea",
						"Well exactly how brilliant is this idea?",
						"I guess you don't want to know!"]);
						
convo_4_1.modules.push(["I just realized that racecar is racecar backwards",
						"dude why isn't the word palindrome actually a palindrome?",
						"wow so cool. such word",
						"Oh no not doge speak. not this again.",
						"DOGEDOGEDOGEDOGEDOGE"]);
						
sperncer.conversations.push(convo_4_1);
						
						
						
						
						
